...
【动作弧三段式（核心，必须严格遵守）】每个镜头的动作必须设计成完整动作弧，actionDescription 必须按以下结构撰写，且显式标注分段：
【起始状态】画面开始瞬间：人物姿势、位置、表情、道具状态（静态可描述的明确状态）
【动作过程】镜头主体动作：具体动作动词+动作路径+动作幅度（动作要有明显位移/变化，禁止"只是站在那里"这类无动作过程）
【结束状态】画面结束瞬间：动作完成后的明确结果状态（与起始状态必须有可辨认的差异，如 站→坐、抬鞭→鞭已抽下、走→停）
- 动作弧三段之间必须有明确的视觉差异，差异越大视频动作越明显
- 【禁止】镜头只有单一静态状态（如"人物站在墙角，表情沉重"）而没有动作过程；每个镜头至少有一个可见的肢体/表情/位置变化
- 短剧节奏：5秒镜头=完整动作弧（约1秒起始→3秒主体动作→1秒收尾），动作幅度充分、节奏紧凑，禁止缓慢微动、禁止几乎静止的"慢吞吞"画面
...
- phase: 所属阶段编号（1-4），本集必须严格分为4个阶段
- phaseName: 阶段名称（如"开场引入""矛盾升级""高潮爆发""收束悬念"，4个阶段的名称需连贯递进）
...
- actionDescription: 画面中的动作和场景描述（导演级细节：姿态/肢体/表情/道具/光影/动态过程/声音，40-120字）。必须按【动作弧三段式】撰写：以【起始状态】【动作过程】【结束状态】三个小节分段，起始与结束状态必须有明确视觉差异
- firstFrameDescription: 镜头首帧画面描述（=动作弧的【起始状态】展开：人物姿势、位置、表情、道具、光影，20-60字）。用于生成首帧关键帧，必须是一个明确的静态画面
- lastFrameDescription: 镜头尾帧画面描述（=动作弧的【结束状态】展开：动作完成后的结果画面，人物姿势、位置、表情、道具、光影，20-60字）。用于生成尾帧关键帧，必须与 firstFrameDescription 有可辨认的状态差异
...
4. actionDescription 必须按导演级细节标准展开（40-120字）：覆盖人物姿态与动作、面部表情、关键道具状态、光影氛围、声音线索，禁止笼统单句；且必须按【动作弧三段式】撰写，显式分段【起始状态】【动作过程】【结束状态】，起始与结束必须有明确视觉差异
4b. 每个镜头必须同时输出 firstFrameDescription（首帧画面=起始状态）和 lastFrameDescription（尾帧画面=结束状态），两段描述必须对应不同的画面状态（如"举鞭欲抽"→"鞭已抽下"），确保首尾帧视频生成时有足够的动作空间；禁止首尾帧描述雷同
5. 镜头运动要符合场景情绪：动作用手持/推镜，对话用固定/稳定器，情感用慢推；合理运用推、拉、摇、移等技巧
6. 每个镜头必须标注subject（镜头主体），主角出场必须给到主角镜头，不能被配角抢镜
7. 前后镜头角色位置和状态要连贯，服装发型一致
8. 按剧本顺序排列
9. 【4阶段划分】本集必须按剧情发展划分为 4 个阶段（phase 1-4）：阶段1开场引入（建立场景和人物）、阶段2矛盾升级（冲突推进）、阶段3高潮爆发（核心冲突顶点）、阶段4收束悬念（情绪回落+钩子）。镜头按剧情顺序依次归入对应阶段，每个阶段约占总镜头数的1/4，同一阶段的镜头在画面风格和情绪基调上保持连贯
10. 分镜序号连续，4个阶段内镜头数大致均衡（每个阶段镜头数相差不超过3个）
11. durationSeconds 根据 pace 自动匹配（fast=2, normal=4, slow=6, slow_motion=3），整体控制在单集约3秒
12. 主画面描述严格按"主体+景别+视角+构图+氛围光感"撰写，方便AI出图和生视频
${params.characters && params.characters.length > 0
  ? '13. 角色外貌与服装严格按上方【角色定妆信息】引用角色名，不得自行更改外观'
  : '13. 无需描述服装造型（已在资产库中定义），确保分镜适配AI元素的添加'}
14. 遵守制作合规红线：无文字Logo品牌、医疗仅表现状态、人物原创虚构、无血腥暴力与未成年人
